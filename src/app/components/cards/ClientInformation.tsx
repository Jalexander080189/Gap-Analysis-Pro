...
const ClientInformation: React.FC<ClientInformationProps> = ({ data, setData }) => {
  ...
  return (
    <div className="card">
      {!data.showBack ? (
        <div>
          {/* Ultra-compact LinkedIn-style profile header */}
          <div className="relative">
            {/* Cover photo - extremely reduced height */}
            <div className="relative h-16 bg-black overflow-hidden">
              <div className="absolute inset-0 bg-cover bg-center" 
                   style={{ backgroundImage: `url(${data.coverImage || defaultCoverImage})` }}>
                {/* Edit cover button - tiny and subtle */}
                <button 
                  type="button"
                  onClick={() => coverInputRef.current?.click()}
                  className="absolute top-1 right-1 bg-white bg-opacity-75 p-0.5 rounded-full hover:bg-opacity-100 transition-all"
            ...
            {/* Profile image - tiny size */}
            <div 
              className={`absolute left-2 -bottom-4 w-10 h-10 rounded-full border-1 border-white bg-gray-200 overflow-hidden shadow-sm ${dragActive ? 'ring-1 ring-blue-500' : ''}`}
            ...
            {/* Edit profile button - tiny and subtle */}
            <button
              type="button"
              onClick={toggleEdit}
              className="absolute top-1 right-4 bg-white bg-opacity-75 p-0.5 rounded-full hover:bg-opacity-100 transition-all"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-2 w-2 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
              </svg>
            </button>
          </div>

          {/* Ultra-compact profile information */}
          <div className="pt-6 px-2 pb-1">
            <div className="flex flex-col">
              {/* Name with verification badge - ultra-compact */}
              <div className="flex items-center">
                <h1 className="text-sm font-bold text-gray-900 truncate">{data.companyName || 'Company Name'}</h1>
                <svg className="ml-1 h-3 w-3 text-gray-500" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M9 12L11 14L15 10M21 12C21 16.9706 16.9706 21 12 21C7.02944 21 3 16.9706 3 12C3 7.02944 7.02944 3 12 3C16.9706 3 21 7.02944 21 12Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </div>
              
              {/* Job title and company - ultra-compact */}
              <p className="text-xs text-gray-700 truncate">
                {data.contacts && data.contacts[0]?.title ? data.contacts[0].title : 'Job Title'} • {data.industryType || 'Industry'}
              </p>
              
              {/* Website and social links - ultra-compact */}
              <div className="mt-0.5 flex flex-wrap gap-1 text-xs">
                ...
                {data.companyFacebookURL && (
                  <a 
                    ...
                    className="flex items-center text-blue-600 hover:underline"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-2 w-2 mr-0.5" viewBox="0 0 24 24" fill="currentColor">
            ...
            <div className="mt-1">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-medium text-gray-700">Contacts</h3>
                {data.contacts && data.contacts.length > 0 && (
                  <span className="text-xs text-gray-500">{data.contacts.length}</span>
                )}
              </div>
              
              {data.contacts && data.contacts.length > 0 ? (
                <div className="mt-0.5 text-xs">
                  {data.contacts.slice(0, 1).map((contact, index) => (
                    <div key={index} className="flex items-center justify-between">
                      <span className="font-medium truncate">{contact.name}</span>
                      {contact.email && (
                        <a href={`mailto:${contact.email}`} className="text-blue-600 hover:underline truncate ml-1">
                          {contact.email}
                        </a>
                      )}
                    </div>
                  ...
                  {data.contacts.length > 1 && (
                    <div className="text-xs text-gray-500 italic">+{data.contacts.length - 1} more</div>
                  )}
                </div>
              ) : (
            ...
            {/* Business Overview Section - ultra-compact */}
            <div className="mt-1">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-medium text-gray-700">Business Overview</h3>
                <button
                  type="button"
              ...
              {showBusinessOverview && (
                <div className="mt-0.5 bg-gray-50 p-1 rounded text-xs" dangerouslySetInnerHTML={{ __html: data.businessDescription || '' }} />
              )}
            </div>
          </div>
          
          {/* Social interaction buttons - ultra-compact */}
          <div className="px-2 py-1 border-t border-gray-200 flex items-center space-x-2">
        ...
        <div className="p-2">
          <div className="flex justify-between items-center mb-2">
            ...
            <button
              type="button"
              onClick={toggleEdit}
              className="px-1.5 py-0.5 text-xs bg-blue-600 text-white font-medium rounded hover:bg-blue-700 transition-colors"
            >
              Save
            </button>
          </div>
          ...
          <div className="mb-2">
            <h3 className="text-xs font-medium text-gray-900 mb-1">Company Information</h3>
            ...
            <div className="grid grid-cols-2 gap-1 mb-1">
              <div>
                ...
                <div 
                  className={`w-10 h-10 rounded-full border border-dashed border-gray-300 flex items-center justify-center bg-gray-50 ${dragActive ? 'border-blue-500 bg-blue-50' : ''}`}
                  onDragEnter={handleDrag}
                  onDragLeave={handleDrag}
                  onDragOver={handleDrag}
                  onDrop={handleDrop}
              ...
              <div>
                ...
                <div 
                  className="w-full h-10 border border-dashed border-gray-300 flex items-center justify-center bg-gray-50"
                  onClick={() => coverInputRef.current?.click()}
                  style={{ cursor: 'pointer' }}
                >
                  {data.coverImage ? (
          ...
          <div className="mb-2">
            <div className="flex justify-between items-center mb-1">
              <h3 className="text-xs font-medium text-gray-900">Contacts ({(data.contacts || []).length}/5)</h3>
              {(data.contacts || []).length < 5 && (
                <button
                  type="button"
                  onClick={addContact}
                  className="px-1 py-0.5 text-xs bg-blue-600 text-white rounded hover:bg-blue-700 flex items-center"
                >
            ...
            {data.contacts && data.contacts.length > 0 ? (
              <div className="space-y-1">
                {data.contacts.map((contact, index) => (
                  <div key={index} className="bg-gray-50 p-1 rounded relative text-xs">
                    <button
                      type="button"
              ...
              <div className="bg-gray-50 p-1 rounded text-center text-xs">
                <button
                  type="button"
                  onClick={addContact}
                  className="px-1 py-0.5 bg-blue-600 text-white rounded hover:bg-blue-700 text-xs inline-flex items-center"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-2 w-2 mr-0.5" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clipRule="evenodd" />
                  </svg>
          ...
          <div className="mb-2">
            <h3 className="text-xs font-medium text-gray-900 mb-1">Business Overview</h3>
            ...
            {typeof window !== 'undefined' && (
              <ReactQuill
                ...
                modules={modules}
                className="bg-white border border-gray-300 rounded text-xs h-16"
